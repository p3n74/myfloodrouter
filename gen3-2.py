...
# 5. Get street names, OSM way IDs, and detailed OSM data along the route
route_details = []
for i in range(len(shortest_path) - 1):
    ...
    # Get the OSM way ID
    way_id = edge_data[0].get('osmid', None)  # OSM way ID
    ...
    # Store the data in a dictionary, using the OSM way ID instead of node IDs
    route_details.append({
        'way_id': way_id,  # Use OSM way ID here
...
with open('t307_route_streets_with_ways.json', 'w') as json_file:
...
print("Route data with OSM way IDs has been saved to 't307_route_streets_with_ways.json'")
